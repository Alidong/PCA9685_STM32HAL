...
static void PCA9685_write_onebyte(uint8_t startAddress, uint8_t data);
static void PCA9685_write_bytes(uint8_t startAddress, uint8_t *data,uint8_t size);
static uint8_t PCA9685_adrr=0;
static void delay(uint16_t ms);
void PCA9685_Reset(void)
{
    PCA9685_write_onebyte(PCA9685_MODE1,MODE1_RESTART);
    delay(10);
}
...
void SetPWMFreq(float freq)
{
    ...
    PCA9685_write_onebyte(PCA9685_MODE1, newmode); // go to sleep
    PCA9685_write_onebyte(PCA9685_PRESCALE, prescale); // set the prescaler
    //printf("set the prescaler=%d\r\n",prescale);
    PCA9685_write_onebyte(PCA9685_MODE1, oldmode);
    delay(5);
    // This sets the MODE1 register to turn on auto increment.
    PCA9685_write_onebyte(PCA9685_MODE1, oldmode | MODE1_RESTART | MODE1_AI);
...
void SetPWM(uint8_t num,uint16_t on,uint16_t off)
{
    ...
    //   printf("SetPWM Err! %d\r\n",write_status);  
    PCA9685_write_bytes((PCA9685_LED0_ON_L + 4 * num),tx_buff,4);
}
...
static void PCA9685_write_onebyte(uint8_t startAddress, uint8_t data) {
    ...
    if(write_status!=HAL_OK)
      printf("PCA9685_write_onebyte Err! %d\r\n",write_status);
}
static void PCA9685_write_bytes(uint8_t startAddress, uint8_t *data,uint8_t size) {
    //Send address to start reading from. 
    HAL_StatusTypeDef write_status=HAL_OK;
    write_status=HAL_I2C_Mem_Write(&hi2c1,PCA9685_adrr,startAddress,I2C_MEMADD_SIZE_8BIT,data,size,1000);
    if(write_status!=HAL_OK)
      printf("PCA9685_write_bytes Err! %d\r\n",write_status);
}
static void delay(uint16_t ms){
    HAL_Delay(ms);
}
